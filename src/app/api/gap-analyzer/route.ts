...
const gapAnalyzerSchema = z.object({
  ...
  starMethodFeedback: z.string().describe("تقييم لمدى استخدام المرشح لمنهجية STAR في إجاباته السلوكية، وهل أعطى أمثلة واقعية أم كانت إجاباته نظرية؟"),
  idealAnswers: z.array(
    z.object({
      question: z.string().describe("السؤال الذي طُرح في المقابلة"),
      userAnswerSummary: z.string().describe("ملخص قصير جداً لإجابة المرشح (أو ذكر أنها كانت ضعيفة/قصيرة)"),
      idealAnswer: z.string().describe("الإجابة النموذجية أو الهيكل الأفضل الذي كان ينبغي للمرشح الرد به")
    })
  ).describe("قائمة بـ 2 إلى 3 أسئلة رئيسية واجه فيها المرشح صعوبة مع توفير الإجابة النموذجية لها")
});
  ...
  try {
    ...
    const { object } = await generateObject({
      ...
      prompt: `
        ...
        6. تقييم مفصل لاستخدام المرشح لمنهجية STAR (Situation, Task, Action, Result).
        7. الإجابات النموذجية للأسئلة التي تعثر فيها المرشح أو أعطى إجابات قصيرة جداً.
